...
if ($isLoggedIn) {
    ...
    $visitsResult = $conn->query("SELECT COUNT(*) as total FROM site_visits");
    $totalVisits = $visitsResult ? (int) $visitsResult->fetch_assoc()['total'] : 0;

    $todayResult = $conn->query("SELECT COUNT(*) as total FROM site_visits WHERE DATE(visited_at) = CURDATE()");
    $todayVisits = $todayResult ? (int) $todayResult->fetch_assoc()['total'] : 0;
}
?>
<!DOCTYPE html>
<html lang="en">
...
<?php else: ?>
  ...
  <div style="display:flex;gap:14px;margin-bottom:30px;">
    <div style="background:#fff;border:1px solid #eee;border-radius:10px;padding:16px 22px;flex:1;">
      <div style="font-size:0.8rem;color:#6b543a;text-transform:uppercase;letter-spacing:1px;">Total Visits</div>
      <div style="font-size:1.8rem;font-weight:700;"><?php echo number_format($totalVisits); ?></div>
    </div>
    <div style="background:#fff;border:1px solid #eee;border-radius:10px;padding:16px 22px;flex:1;">
      <div style="font-size:0.8rem;color:#6b543a;text-transform:uppercase;letter-spacing:1px;">Today</div>
      <div style="font-size:1.8rem;font-weight:700;"><?php echo number_format($todayVisits); ?></div>
    </div>
  </div>
